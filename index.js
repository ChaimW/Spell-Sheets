...
function loadAbilities() {
    ...
    abilitiesRequest.onreadystatechange = function() {
        if (abilitiesRequest.readyState == 4) {
            setInfo("Loading abilities, connected...");
            if (abilitiesRequest.status >= 200 && abilitiesRequest.status < 400){
                ...
                console.log("Server reached, returned error code " + abilitiesRequest.status, true);
            }
        } else {
            setInfo("Loading abilities, stand by...");
        }
    };
    
    abilitiesRequest.onerror = function() {
        console.log("Connection error", true);
    };
...
function unlock() {
    ...
    infoRequest.onreadystatechange = function() {
        if (infoRequest.readyState == 4) {
            setInfo(infoRequest.responseText);
        }
    };
...
function loadClass() {
	...
	classRequest.onreadystatechange = function() {
		if (classRequest.readyState == 4) {
            setInfo("Loading class, connected...");
            if (classRequest.status >= 200 && classRequest.status < 400){
				...
				currentPlayer = new Player(className, classLevel);
				
				update();
            } else {
                console.log("Server reached, returned error code " + classRequest.status, true);
            }
        } else {
            setInfo("Loading class, stand by...");
        }
	};
